                    ...
                    <div class="widget-header">
                        <div class="row">
                            <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                <h4 class="text-center text-success">Edit  Group Student</h4>
                            </div>
                        </div>
                    </div>
                    <div class="widget-content widget-content-area">
                        <form action="{{ route('admin.group_students.update',$groupStudent->id) }}" method="post">
                            @csrf
                            @method('PUT')
                                    ...
                                    <select class="form-control basic"
                                        ...
                                        @foreach ($students as $student)
                                            <option value="{{ $student->id }}"
                                                {{ $groupStudent->student_id == $student->id ? 'selected' : '' }}>
                                                {{ $student->name }}</option>
                                        @endforeach
                                    </select>
                                    @error("student_id")
                                    ...
                                    <select class="form-control basic"
                                        ...
                                        @foreach ($groups as $group)
                                            <option value="{{ $group->id }}"
                                                {{ $groupStudent->group_id == $group->id ? 'selected' : '' }}>
                                                {{ $group->from }} :
                                                {{ $group->to }}
                                            </option>
                                        @endforeach
